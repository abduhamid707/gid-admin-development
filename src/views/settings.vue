<template>
    <div class="wrapper">
        <Header :header_title="title" :main_crumb="[{title:title,path:'/'}]" />
        <div class="settings">
            <div class="settings-tabs">
                <div :class="{active:tab==1}" @keydown.ctrl.116="tab=1" @click="changeTab(1)" class="settings-tab">Profillar <span>ctrl + F5</span></div>
                <div :class="{active:tab==2}" @keydown.ctrl.117="tab=2" @click="changeTab(2)" class="settings-tab">Arizalar <span>ctrl + F6</span></div>
                <!-- <div :class="{active:tab==3}" @keydown.ctrl.118="tab=3" @click="changeTab(3)" class="settings-tab">Mening profilim <span>ctrl + F7</span></div> -->
                <div :class="{active:tab==3}" @keydown.ctrl.118="tab=3" @click="changeTab(3)" class="settings-tab">Dastur ko‘rinishi <span>ctrl + F7</span></div>
            </div>
            <div v-if="tab==1 && ($store?.user?.role == 'super')" class="set-list">
                <div class="set-count">
                    <p>Barcha foydalanuvchilar soni:</p>
                    <span>{{tableProfiles?.count||0}}</span>
                </div>
                <div class="set-name">
                    ...
                    <div class="table">
                        <div class="thead">
                            <div class="tr">
                                ...
                                <div class="td">Profil holati</div>
                                <div class="td">Roli</div>
                                <div class="td">So‘nggi foydalanish vaqti</div>
                            </div>
                            <div class="thead-filter">
                                ...
                                <div class="td">
                                    <el-select @change="changeFilter" v-model="filter" collapse-tags clearable filterable :placeholder="$t('selectCat')">
                                        <el-option
                                ...
                                <div class="td">
                                    <el-select @change="changeFilter" v-model="filter" collapse-tags clearable filterable :placeholder="$t('selectSta')">
                                        <el-option
                                        v-for="item in status"
                                        :key="item.id"
                                        :label="item.name?.[$i18n.locale]"
                                ...
                                <div class="td">
                                    <el-select @change="changeFilter" v-model="filter.level" multiple collapse-tags clearable filterable :placeholder="$t('selectLang')">
                                        <el-option
                                        v-for="item in status"
                                        :key="item.id"
                                        :label="item.name?.[$i18n.locale]"
                        ...
                        <div class="tbody" v-for="el,index in tableProfiles.rows" :key="index">
                            <div class="td">{{index+1}}</div>
                            <div class="td">
                                <!-- <img :src="el?.img" alt=""> -->
                                {{el?.first_name}} {{el?.sur_name}} {{el?.mid_name}}
                            </div>
                            <div class="td">{{el?.status == 'active' ? 'Faol' : 'Faol emas'}}</div>
                            <div class="td">{{el?.role == 'user' ? 'Foydalanuvchi' : el?.role == 'moderator' ? 'Moderator' : el?.role == 'admin' ? 'Admin' : '-'}}</div>
                            <div class="td">
                                {{el?.createdAt|dateFilter}}
                                <Icons @click.native="openModal(el)" size="middle" icon="pen" color="black"/>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div v-if="tab==2 && ($store?.user?.role == 'super')" class="set-list">
                <div class="set-count">
                    <p>Barcha arizalar soni:</p>
                    <span>{{tableAppls?.length}}</span>
                </div>
                <div class="set-name">
                    <p>Arizalar</p>
                </div>
                    ...
                    <div class="table">
                        <div class="thead">
                            <div class="tr">
                                <div class="td">№</div>
                                <div class="td">F.I.Sh</div>
                                <div class="td">Ariza kelib tushgan sana</div>
                            ...
                            <div class="thead-filter">
                                ...
                                <div class="td">
                                    <el-select @change="changeFilter" v-model="filter.category" collapse-tags clearable filterable :placeholder="$t('selectCat')">
                                        <el-option
                                        v-for="item in status"
                                        :key="item.id"
                                        :label="item.name?.[$i18n.locale]"
                                ...
                                <div class="td">
                                    <el-select @change="changeFilter" v-model="filter.activity" collapse-tags clearable filterable :placeholder="$t('selectSta')">
                                        <el-option
                                        v-for="item in status"
                                        :key="item.id"
                                        :label="item.name?.[$i18n.locale]"
                        ...
                        <div class="tbody" v-for="el,index in tableAppls" :key="index">
                            <div class="td">{{index+1}}</div>
                            <div class="td">
                                <!-- <img :src="el?.img" alt=""> -->
                                {{el?.first_name}} {{el?.sur_name}} {{el?.mid_name}}
                            </div>
                            <div class="td">{{el?.createdAt|dateFilter}}</div>
                            <div class="td">{{el?.app_status == 'checking' ? 'Tekshirilmoqda' : el?.app_status == 'cancelled' ? 'Bekor qilindi' : 'Tasdiqlangan'}}</div>
                            <div class="td">
                                <div class="table-act-set">
                                    <button @click="cancel(el)" class="cancel">Bekor qilish</button>
                                    <button @click="save(el)" class="active">Qabul qilish</button>
            ...
            <!-- <div v-if="tab==3" class="profile-mode">
                <div class="profile-image">
                    <label for="">Hududiy boshqarmalari</label>
                    <div class="profile-img">
                        <img src="@/assets/image/Frame555.png" alt="">
                        <div class="profile-img-edit">
                            <Icons color="white" icon="editPen" size="custom" width="38" height="38"/>
                            <Icons color="red" icon="delete" size="custom" width="38" height="38"/>
                        </div>
                    </div>
                </div>
            ...
            <div v-if="tab==3" class="set-mode">
                <div class="toggle-mode">
                    <label for="">Hududiy boshqarmalari</label>
                    <button @click="toggleMode" class="toggleMode" :aria-pressed="mode" ref="button" title="Toggle Dark Mode">
                ...
                <div class="toggle-mode">
                    <label for="">Tillar</label>
                    <el-select @change="changeLang($event)" v-model="lang" placeholder="Til tanlang">
                        <el-option
                        v-for="item in langs"
                        :key="item.id"
                        :label="item.name"
        ...
        <Modal :user_info="user_info" v-if="oModal" @modalShow="modalShow"/>
        <ModalRole :user_info="user_info2" v-if="oModal2" @modalShow="modalShow2"/>
...
import ModalRole from '@/components/popup/modalRole.vue'
export default {
    data(){
        return{
            title:'Ma’lumotnoma',
    ...
    methods:{
        ...
        toggleMode(){
            this.mode = !this.mode
            JSON.stringify(localStorage.setItem('mode',this.mode ? true : false))
            document.body.setAttribute("data-dark-mode", this.mode ? true : false);
        },
        save(e){
            this.user_info2 = e
            this.oModal2 = true
        ...
        cancel(el){
            return this.$axios({
            method: "PUT",
            url: "adminapplicants/"+el?.user_id,
    ...
    async mounted(){
        ...
        if(this.$store?.user?.role == 'super'){
            await this.$axios.get('/adminapplicants').then((res)=>{
                if(res && res.data){
                    this.tableAppls = res && res.data && res.data.result && res.data.result.users
                }
            });
            await this.$axios.get('/users').then((res)=>{
                if(res && res.data){
                    this.tableProfiles = res && res.data && res.data.result && res.data.result
                }
            });
        }
